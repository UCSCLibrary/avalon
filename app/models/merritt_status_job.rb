...
class MerrittStatusJob < Struct.new(:pid,:batchID)
  ...
  def merritt_check_status(:pid,:batchID)
    batch_uri = "https://merritt.cdlib.org/istatus/bid/"+batchID+'/localfull/"'+pid+'"'
    logger.debug('batch_uri:'+batch_uri)
    ...
    response["rows"].each do |job|
      logger.debug('Job id:'+job["id"]+" status: "+job["value"])
      case job["value"]
      when "COMPLETED"
